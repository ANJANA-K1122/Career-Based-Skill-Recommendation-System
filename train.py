...
# Load the dataset 
df = pd.read_csv("Final_Corrected_Career_Skills_Dataset.csv")
...
unique_labels = sorted(set(int(label) for label in (y_test.tolist() + y_pred.tolist()) if 0 <= int(label) < 
len(label_encoder.classes_))) 
# Use unique_labels directly for target_names 
print(classification_report(y_test, y_pred, target_names=label_encoder.classes_[unique_labels])) # 
Pass filtered labels to target_names 
...
plt.show() 
# Save the model 
model.save("model/career_lstm_model.h5")